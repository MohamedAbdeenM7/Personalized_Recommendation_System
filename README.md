# 🛒 Personalized Recommendation System
## Amazon Electronics Dataset

---

## 👥 Team
- | Student |          Name            |       Role       |    Milestone     |
- |-----------|------------------------|------------------|
- | Student 1 | Mohamed Abdeen Mostafa | Data Engineer    | M1 - Data & EDA  |
- | Student 2 |                        | ML Engineer      | M2 - Models      |
- | Student 3 |                        | Backend Dev      | M3 - API & Azure |
- | Student 4 |                        | MLOps Engineer   | M4 - Monitoring  |
- | Student 5 |                        | PM & Tech Writer | M5 - Docs        |

---

## 📁 Project Structure
### recommendation-system/
- ├── data/          # Raw and processed data
- ├── notebooks/     # Colab notebooks per student
- ├── src/           # Source code
- ├── tests/         # Unit tests
- ├── deployment/    # Docker & Azure configs
- ├── configs/       # Model & Azure settings
- └── docs/          # Reports & documentation

---

## 🚀 How to Run

- !git clone https://github.com/MohamedAbdeenM7/recommendation-system.git
- %cd recommendation-system
- !pip install -r requirements.txt

---

## 📊 Dataset
- Source: Amazon Product Reviews — Electronics
- Link: https://cseweb.ucsd.edu/~jmcauley/datasets/amazon_v2/
- Type: Ratings Only (user_id, item_id, rating, timestamp)
- Category: Electronics

---

## 📌 Milestones
| Milestone | Description | Status |
|-----------|-------------|--------|
| M1 | Data Collection, Preprocessing & EDA | 🔄 In Progress |
| M2 | Model Development & Evaluation | ⏳ Pending |
| M3 | Cloud Deployment & Integration | ⏳ Pending |
| M4 | MLOps & Monitoring | ⏳ Pending |
| M5 | Final Documentation & Presentation | ⏳ Pending |
